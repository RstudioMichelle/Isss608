...
In this hands-on exercise, the focus is on visualising and analysing text data using R.
...
The following R packages are required for handling, processing, wrangling, analysing and visualising text data:
...
Instead of counting individual word, we can also count words within by newsgroup.
...
In this step below, `wordcloud()` of **wordcloud** package is used to plot a static wordcloud.
...
A DT table can be used to complement the visual disco
...
In this step below, a bigram data frame is created by using `unnest_tokens()` of tidytext.
...
The cstep is used to count and sort the bigram data frame ascendingly.
...
The step below is used to seperate the bigram into two words.
...
The step is used to plot the network graph.
